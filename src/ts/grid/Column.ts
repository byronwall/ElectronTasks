...
export class Column {
  ...
  constructor(config) {
    ...
    for (var p in props)
      this[p] =
        typeof config == "undefined" || typeof config[p] == "undefined"
          ? props[p]
          : config[p];
  }
  ...
  getOptionValuesForRender(rowIndex) {
    if (!this.enumProvider) {
      console.log(
        "getOptionValuesForRender called on column " +
          this.name +
          " but there is no EnumProvider"
      );
      return null;
    }
    var values = this.enumProvider.getOptionValuesForRender(
      this.editablegrid,
      this,
      rowIndex
    );
  ...
  getOptionValuesForEdit(rowIndex) {
    if (!this.enumProvider) {
      console.log(
        "getOptionValuesForEdit called on column " +
          this.name +
          " but there is no EnumProvider"
      );
      return null;
    }
    var values = this.enumProvider.getOptionValuesForEdit(
      this.editablegrid,
      this,
      rowIndex
    );
  ...
  isValid(value) {
    for (var i = 0; i < this.cellValidators.length; i++)
      if (!this.cellValidators[i].isValid(value)) return false;
    return true;
  }
