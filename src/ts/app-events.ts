...
function setupSettingsRelatedEvents() {
  ...
  $("#btnShouldShowTagsForComplete").on("click", function(ev) {
    ...
    var shouldShowComplete = $("#btnShouldShowTagsForComplete").hasClass(
      "active"
    );
...
function setupActionPanelButtonEvents() {
  ...
  $("#gridList").on("click", ".btnDelete", function(ev) {
    ...
    if (currentTask.isProjectRoot) {
      ...
      if (projectCount === 1) {
        console.log(
          "task cannot be deleted, since it is the last project root"
        );
        return false;
      }
...
function setupMousetrapEvents() {
  ...
  Mousetrap.bind(["alt+up", "alt+down"], function(e: KeyboardEvent, combo) {
    ...
    if (target.tagName === "INPUT") {
      ...
      renderGrid();
      grid.editCell(
        grid.getRowIndex(currentTask.ID),
  ...
  Mousetrap.bind(
    ["ctrl+alt+right", "ctrl+alt+left", "ctrl+alt+up", "ctrl+alt+down"],
    function(ev: KeyboardEvent, combo) {
      var target = ev.target as HTMLElement;
      if (
        target.tagName === "INPUT" &&
        $(ev.target).parents("#gridList").length
      ) {
        ...
        do {
          columnIndex =
            (columnIndex + colChange + grid.getColumnCount()) %
            grid.getColumnCount();
  ...
  Mousetrap.bind(["shift+c", "alt+shift+c"], function(e, combo) {
    ...
    var validShortcut = isKeyboardInEditor(e.target)
      ? combo.indexOf("alt") > -1
      : true;
    if (validShortcut) {
  ...
  Mousetrap.bind(["alt+/", "/", "shift+/", "alt+shift+/"], function(
    e: KeyboardEvent,
    combo
  ) {
...
function applyEdit(element, shouldCancel = false) {
  var editor =
    element instanceof Column ? element.cellEditor : element.celleditor;
  ...
  if (shouldCancel) {
    ...
    if (
      editor.applyEditing(element.element, editor.getEditorValue(element)) ===
      false
    ) {
...
function createNewProject() {
  ...
  grid.editCell(
    grid.getRowIndex(newProjectTask.ID),
    grid.getColumnIndex("description")
  );
}
