...
export class TaskList {
  ...
  determineIfTasksAreVisible() {
    ...
    _.each(this.tasks, function(task) {
      var searchResult =
        self.searchTerm === "" || task.isResultForSearch(self.searchObj);
      var showBecauseComplete = self.shouldHideComplete
        ? !task.isComplete
        : true;
  ...
  recurseChildren(task: Task, indentLevel, tasksOut) {
    ...
    var subOrder = _.map(task.childTasks, function(childTaskId) {
      return {
        sort: self.tasks[childTaskId][self.activeSortField],
        id: childTaskId
      };
    });
  ...
  removeTask(ID) {
    ...
    if (taskToDelete.parentTask !== null) {
      ...
      parentTask.childTasks = parentTask.childTasks.concat(
        taskToDelete.childTasks
      );
  ...
  isDefaultList() {
    ...
    if (this.tasks[2].description === "new project") {
      if (
        Object.keys(this.tasks).length === 2 &&
        this.tasks[3] !== undefined &&
        this.tasks[3].description === "new task"
      ) {
        return true;
      }
